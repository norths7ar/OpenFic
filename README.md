...
写作、背景设定、角色、提纲和笔记共用文档历史与项目回收站。编辑器工具栏的“历史版本”可预览并恢复标题和正文；连续手工编辑按 5 分钟合并恢复点，每篇保留最近 100 个。AI 修改、导入覆盖、应用待审变更及恢复操作会在内容变化前单独留点；恢复不改变文档当前的文件夹和可见性。历史从功能启用后的修改开始记录，不追溯生成过去的版本。

删除条目后可通过标签栏右侧的“回收站”预览、恢复或永久删除。回收站不自动清空；恢复沿用条目 ID，原文件夹或卷不存在时放回对应类型的根目录，同名等冲突会阻止覆盖。历史归属于文档，不随讨论任务删除而消失；永久删除回收站条目或删除整个项目会清理对应记录。

这些记录保存在应用数据库中，包含在应用完整备份内；项目资料包与 Markdown 源包只交换当前资料，不携带文档历史或回收站。现有讨论消息的“回滚到此处”仍是独立的 AI 任务回滚入口。

...
- **Markdown 源包**：面向人类可读的文档仓库，根据 `openfic-import.yaml` 在背景设定、角色、提纲、笔记和正文之间做语义级映射。章节默认位于 `正文/<卷标题>/<章标题>.md`，无卷章节直接放在 `正文/`；只有名称冲突时添加 ID 后缀。章节 frontmatter 保存稳定身份、所属卷和顺序，改文件名不会改变身份，不应手动删除这些元数据。

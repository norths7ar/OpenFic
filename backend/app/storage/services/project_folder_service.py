...
async def delete_folder(session: AsyncSession, folder_id: str) -> None:
    ...
    model_by_scope = {
        "discussion": Task,
        "world": WorldInfoEntry,
        "character": Character,
        "writing": Chapter,
        "note": Note,
        "outline": Note,
    }
    model = model_by_scope.get(folder.scope)
    if model is None:
    ...
    # Preserve root order, then append the former folder's contents as one ordered block.
    project_filter = (
        model.world_info_id.in_(
            select(WorldInfo.id).where(WorldInfo.project_id == folder.project_id)
        )
        if folder.scope == "world"
        else model.project_id == folder.project_id
    )
    scope_filter = (
        model.document_type == folder.scope if folder.scope in {"note", "outline"} else True
    )
    roots = (
        (
            await session.execute(
                select(model).where(project_filter, scope_filter, getattr(model, field).is_(None))
            )
        )
        .scalars()
        .all()
    )
    children_query = select(model).where(getattr(model, field) == folder.id)
    children_query = (
        children_query.order_by(model.order, model.id)
        if hasattr(model, "order")
        else children_query.order_by(model.created_at, model.id)
    )
    children = (await session.execute(children_query)).scalars().all()
    next_order = max((getattr(item, "order", 0) for item in roots), default=0) + 1
    ...
    if folder.scope == "writing":
        await session.execute(
            ...
            .where(ChapterSummary.volume_id == folder.id)
